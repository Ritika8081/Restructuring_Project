 ...
 * src/lib/filters.ts
 *
 * Collection of signal processing filters. Start with a Notch filter class
 * (exported as `Notch`) but keep the file name generic for additional
 * filter implementations in the future.
 */
...
export class Notch {
    ...
    /**
     * Set the sampling rate (Hz) used by the filter so the correct
     * coefficient set is selected in `process`.
     */
    ...
    /**
     * Process a single input sample through the notch filter.
     * @param input sample value
     * @param type 1 = 50Hz-range notch, 2 = 60Hz-range notch
     */
